...
def endpoint_in_json(json, key):
    return key in json and "ADDRESS" in json[key] and "PORT" in json[key]
...
def get_parameters_from_sequence(data, validators=[]):
    ...
    if len(data) != len(validators):
        raise ServerExceptions.PARAMETER_ERROR(f"Expected {len(validators)} parameters but received {len(data)}.")
    ...
    for i in range(len(data)):
        ...
        if not validator(param_val):
            raise ServerExceptions.INVALID_PARAMETER(f"Parameter {i + 1} failed validation.")
...
def get_parameters_from_dict(data, *args):
    ...
    for i in range(len(args)):
        ...
        if param in data:
            ...
            raise ServerExceptions.PARAMETER_ERROR(f"Expected parameter '{param}' not received.")

        if not validator(param_val):
            raise ServerExceptions.INVALID_PARAMETER(f"Parameter '{param}' failed validation.")
