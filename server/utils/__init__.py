...
class Endpoint:
    def __init__(self, ip: str, port: int, route: str = None):
        if not ip: self.ip = None
        elif ip.startswith('https://'): self.ip = ip[8:]
        elif ip.startswith('http://'): self.ip = ip[7:]        
        else: self.ip = ip

        self.port = port

        if not route: self.route = None
        elif route == '/': self.route = None
        elif route.startswith('/'): self.route = route[1:]
    ...
    def __iter__(self):
        ...
        if self.port: yield self.port
        if self.route: yield self.route