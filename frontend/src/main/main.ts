  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('stream', (frame: ArrayBuffer) => {
      ...
      frameBlob
        .arrayBuffer()
        .then((frameBuffer) => {
          const videoFrame = new VideoFrame(frameBuffer, {'format' : "RGBX", "displayWidth" : 480, "displayHeight": 640});

          // Send this frame to the other window
          if (mainWindow !== null) mainWindow.webContents.send('frame', videoFrame);
