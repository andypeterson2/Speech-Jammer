...
# TODO: Assure we don't need anything in this file
...
class Alert:
    ...
    def show(self):
        return messagebox.showwarning(self.title,
                                      self.message,
...
class GUIInterface(ABC):  # TODO: Potentially subclass Thread to avoid blocking
    ...
    class Form:
        ...
        def setDefaultValues(self):
            """Return all text boxes in `self.entries`
            to default values specified in `self.fields`"""
            for field in self.fields:
                text_box = self.entries[field[0]]
                text_box.delete(0, END)
                text_box.insert(0, field[1])

    def createForm(self, on_submit: Callable[..., Any],
                   button_text: str = 'Submit', *fields):
        """Pack a form, consisting of rows of labels and fields
        and a submit button at the bottom, into the root window.

        Parameters
        ----------
        on_submit : func
            Function that runs when submit button is pressed. Will be
            passed field entries at time of submission in order
            received in `fields`
        field : tuple
        ...
        def on_submit_(): return on_submit(*form.getEntries())
        button = Button(self.root, text=button_text, command=on_submit_)
        button.pack(side=BOTTOM, padx=5, pady=5)
        # Submits form when enter/return is pressed
        self.root.bind('<Return>', (lambda e: on_submit_()))
...
class InitClientGUI(GUIInterface):
    ...
    def __init__(self):
        ...
        self.form = self.createForm(
            ...
            ('Server IP:', self.DEFAULT_IP), ('Port:', self.DEFAULT_PORT))
...
class MainGUI(GUIInterface):
    """
    GUI to interface with server from. Currently only allows
    user to request to connect to a peer with specified user ID.
    """
    DEFAULT_ID = '12345'
    ...
    def __init__(self, user_id):
        ...
        self.peer_id = None
        self.form = self.createForm(
            self.connectButton, 'Connect', ('User ID:', '12345'))
    ...
    def connectButton(self, user_id):
        if self.verifyID(user_id):
            ...
            alert = Alert(
                'Warning',
                'Please enter a valid user ID (5-digit alphanumeric).')
            alert.show()
            self.form.setDefaultValues()
            self.peer_id = None
